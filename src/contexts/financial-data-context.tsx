...
const getInitialState = (): AllFinancialData => ({
...
export function FinancialDataProvider({ children }: { children: ReactNode }) {
  ...
  const [couplesData, setCouplesData] = useState<CouplesData>(initialCouplesData);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, async (docSnap) => {
        ...
        if (docSnap.exists()) {
            ...
            try {
                 await setDoc(docRef, finalState, { merge: true });
            } catch (e) {
                console.error("Failed to create initial user document", e)
            }
        }
        
        setSimulatorData(finalState.simulatorData);
