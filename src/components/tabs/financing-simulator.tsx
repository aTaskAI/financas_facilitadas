  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card className="bg-primary/10 border-primary/20"><CardHeader><CardTitle className="text-primary">Amortização Extra</CardTitle></CardHeader><CardContent><div><Label className="text-primary/80">Valor Mensal</Label><Input type="number" value={simulatorData.amortizacao} onChange={e => handleInputChange('amortizacao', Number(e.target.value))} /></div><CardDescription className="mt-2 text-primary/70">Valor adicional para quitar mais rápido.</CardDescription></CardContent></Card>
      </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-primary/10 border-primary/20"><CardHeader><CardTitle className="text-primary">Custos Totais</CardTitle></CardHeader><CardContent><div className="flex justify-between text-primary"><span>Custo Normal:</span><span className="font-semibold">{formatCurrency(custoRealNormal)}</span></div><div className="flex justify-between text-green-700 dark:text-green-400"><span>Custo com Amortização:</span><span className="font-semibold">{formatCurrency(custoRealAmortizado)}</span></div></CardContent></Card>
              ...
              <TableBody>
                {visibleRows.map((row, index) => {
                  ...
                  return (
                    ...
                    <TableRow className={row.parcela <= tempoComAmort && (simulatorData.amortizacao || 0) > 0 ? "bg-primary/10" : ""}>
