    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-red-50 rounded-lg"><h4 className="text-sm font-medium text-red-800">Total em Dívidas</h4><p className="text-2xl font-bold text-red-600">{formatCurrency(totalDividas)}</p></div>
          <div className="p-4 bg-green-50 rounded-lg"><h4 className="text-sm font-medium text-green-800">Total Já Pago</h4><p className="text-2xl font-bold text-green-600">{formatCurrency(totalPago)}</p></div>
          <div className="p-4 bg-blue-50 rounded-lg"><h4 className="text-sm font-medium text-blue-800">Total Restante</h4><p className="text-2xl font-bold text-blue-600">{formatCurrency(totalRestante)}</p></div>
          <div className="p-4 bg-indigo-50 rounded-lg"><h4 className="text-sm font-medium text-indigo-800">Progresso Geral</h4><p className="text-2xl font-bold text-indigo-600">{formatPercentage(progressoGeral)}</p></div>
        </CardContent>
      </Card>
                ...
                <div className="mt-4 grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 gap-2">
                  {loan.pagamentos.map((p, i) => (
                    <div
                      ...
                      className={`text-center p-2 rounded-md cursor-pointer border ${p.pago ? 'bg-green-100 border-green-300' : 'bg-slate-50'}`}
                    >
