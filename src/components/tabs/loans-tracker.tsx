...
export function LoansTracker() {
  ...
  const addLoan = () => {
    ...
    const pagamentos: Payment[] = Array.from({ length: newLoan.parcelas }, (_, i) => ({
      numero: i + 1,
  ...
  const togglePayment = (loanId: number, paymentIndex: number) => {
    setLoans(prev => prev.map(loan => {
      if (loan.id === loanId) {
        ...
        // Add payment date when marking as paid
        if(payment.pago) {
            payment.dataPagamento = new Date().toISOString();
        } else {
            delete payment.dataPagamento;
        }
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-red-50 rounded-lg dark:bg-red-900/50"><h4 className="text-sm font-medium text-red-800 dark:text-red-200">Total em Dívidas</h4><p className="text-2xl font-bold text-red-700 dark:text-red-400">{formatCurrency(totalDividas)}</p></div>
          <div className="p-4 bg-green-50 rounded-lg dark:bg-green-900/50"><h4 className="text-sm font-medium text-green-800 dark:text-green-200">Total Já Pago</h4><p className="text-2xl font-bold text-green-700 dark:text-green-400">{formatCurrency(totalPago)}</p></div>
          <div className="p-4 bg-blue-50 rounded-lg dark:bg-blue-900/50"><h4 className="text-sm font-medium text-blue-800 dark:text-blue-200">Total Restante</h4><p className="text-2xl font-bold text-blue-700 dark:text-blue-400">{formatCurrency(totalRestante)}</p></div>
          <div className="p-4 bg-indigo-50 rounded-lg dark:bg-indigo-900/50"><h4 className="text-sm font-medium text-indigo-800 dark:text-indigo-200">Progresso Geral</h4><p className="text-2xl font-bold text-indigo-700 dark:text-indigo-400">{formatPercentage(progressoGeral)}</p></div>
        </CardContent>
      </Card>
